...
# 6. Make an edit to the post with the alter panel.
# 7. Make sure that the raw file for the blog post exists / right content after edit
#==
...
my $post = $t->app->jekyll( 'blog.example.com' )->get_post( '2022-11-27-first-post.markdown' );

is $post->headers->{title}, 'First Post!',           'The title of the post is correct.'; 
is $post->markdown,         'This is my first post', 'The content of the post is correct.'; 

# Make an edit to the post
$t->post_ok( "/dashboard/blog/$blog_id/post/alter", form => {
        mdfile      => '2022-11-27-first-post.markdown',
        postHeaders => $post->headers_as_string,
        postContent => 'This is my first edited post',
    })
    ->status_is( 302 )
    ->header_is( location => "/dashboard/blog/$blog_id/posts" );

# Try to re-read the post we edited and confirm it's correct.
$post = $t->app->jekyll( 'blog.example.com' )->get_post( '2022-11-27-first-post.markdown' );

is $post->headers->{title}, 'First Post!',                  'The title of the post is correct.'; 
is $post->markdown,         'This is my first edited post', 'The content of the post is correct.'; 
